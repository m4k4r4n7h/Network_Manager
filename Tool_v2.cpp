...
void changeNetworkMode() {
    string interfaceName, mode, response;
    ...
    cout << "Do you want to change the interface " << interfaceName << " to monitor mode or managed mode? (monitor/managed): ";
    cin >> mode;
    
    if (mode == "monitor") {
        
        string command = "sudo ifconfig " + interfaceName + " down && sudo iwconfig " + interfaceName + " mode monitor && sudo ifconfig " + interfaceName + " up";
        cout << "\nChanging " << interfaceName << " to monitor mode...\n";
        system(command.c_str());
    } else if (mode == "managed") {
        
        string command = "sudo ifconfig " + interfaceName + " down && sudo iwconfig " + interfaceName + " mode managed  && sudo ifconfig " + interfaceName + " up";
        cout << "\nChanging " << interfaceName << " to managed mode...\n";
        system(command.c_str());
    } else {
        cout << "Invalid mode selected. Please choose either 'monitor' or 'managed'.\n";
    }
}
...
void showHelp() {
    ...
    cout << "change network mode       - Change interface to monitor or managed mode\n";
...
if (geteuid() != 0) {
        cout << "You need to be root to run this program. Exiting...\n";
        return 1;
 }
   commandMap["show private_ip"] = showPrivateIP;
    ...
    commandMap["change network mode"] =  changeNetworkMode;
